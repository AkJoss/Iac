...
def encriptar(texto_plano, key, iv):
    """Encripta el texto plano utilizando AES-256 en modo CBC."""
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
    encryptor = cipher.encryptor()
    
    # Padding para que el texto tenga una longitud múltiplo del
    # tamaño del bloque AES (16 bytes para AES)
...
def desencriptar(texto_cifrado, key, iv):
    """Desencripta el texto cifrado utilizando AES-256 en modo CBC."""
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
...
# 2. Texto original
texto_original = "José Alberto Rocha Munguía"
